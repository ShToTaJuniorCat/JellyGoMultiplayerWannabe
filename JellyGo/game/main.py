...
screen_pixels = np.zeros((SCREEN_WIDTH, SCREEN_HEIGHT))
    ...
    for player_towers in towers_by_player:
        for tower in player_towers:
            ...
            for x in range(tower_rect.left, tower_rect.right):
                for y in range(tower_rect.top, tower_rect.bottom):
                    """
                    This next line seemed a little odd to me - mapping using [y][x] instead of [x][y]?
                    Sounds odd. This line was generated by ChatGPT as I was a little lazy,
                    so I asked why is it this way. I got quite a convincing answer: Basically,
                    single screen_pixels is a 2D array, it's X index represents vertical height (row)
                    and it's Y index represents horizontal width (column).
                    This is opposite to the system used when using tower_rect.left etc.
                    and therefore must be used in opposite order. 
                    """
                    screen_pixels[x][y] = 1  # Mark this pixel as within a tower

jelly_bubble_group = pygame.sprite.LayeredUpdates()

plt.imshow(screen_pixels, cmap='gray', origin='lower')
plt.show()
